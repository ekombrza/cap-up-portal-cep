...
import { getApplicationIdentifier, IApplication } from './application.model';
...
export class ApplicationService {
  public resourceUrl = baseUrlJHipsterApi + 'api/applications';
  public resourceUrlPublic = baseUrlJHipsterApi + 'api/extendedPublic/applications';
  ...
  protected convertDateFromClient(application: IApplication): IApplication {
    return Object.assign({}, application, {
      creationDate: application.creationDate?.isValid() ? application.creationDate.toJSON() : undefined,
      updatedDate: application.updatedDate?.isValid() ? application.updatedDate.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((application: IApplication) => {
        application.creationDate = application.creationDate ? dayjs(application.creationDate) : undefined;
        application.updatedDate = application.updatedDate ? dayjs(application.updatedDate) : undefined;
      });
    }
    return res;
  }
