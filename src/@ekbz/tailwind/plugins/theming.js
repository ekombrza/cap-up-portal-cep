    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--ekbz-${e(key)}`, value], [`--ekbz-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            ]))
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`ekbz.customProps.background.${colorScheme}`);
            const foreground = theme(`ekbz.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--ekbz-${e(key)}`, value], [`--ekbz-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--ekbz-${e(key)}`, value], [`--ekbz-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--ekbz-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--ekbz-on-${name}-rgb), <alpha-value>)`]
                    ])))
                },
                ekbz  : {
                    customProps: {
